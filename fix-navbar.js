...
if (navbarStart === -1) {
  console.log('Navbar function not found');
  process.exit(1);
}
...
let inNavbar = false;
let navbarEnd = -1;
for (let i = navbarStart; i < content.length; i++) {
  if (content[i] === '{') {
    if (!inNavbar) inNavbar = true;
    braceCount++;
  } else if (content[i] === '}') {
    braceCount--;
    if (inNavbar && braceCount === 0) {
      navbarEnd = i + 1;
      break;
    }
  }
...
if (navbarEnd === -1) {
  console.log('Could not find Navbar end');
  process.exit(1);
}

console.log('Navbar function found from', navbarStart, 'to', navbarEnd);
...
function Navbar() {
  ...
  return (
    <nav className={\`fixed w-full top-0 z-50 backdrop-blur-md border-b \${isHeritage ? 'bg-[#141310]/95 border-[#c5a059]/30' : 'bg-[#0a0a0a]/90 border-white/10'}\`}>
      ...
      {/* GDPR Modals */}
      <DataExportModal isOpen={false} onClose={() => {}} />
...
const newContent = content.substring(0, navbarStart) + newNavbar + content.substring(navbarEnd);
fs.writeFileSync('src/app/page.tsx', newContent);
console.log('Navbar function replaced');